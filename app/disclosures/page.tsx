...
export default function DisclosuresPage() {
  ...
  const pillarRequirements = Array.from(
    disclosureThemes.reduce((accumulator, theme) => {
      const current = accumulator.get(theme.pillar) || { ready: 0, requirements: 0 };
      current.ready += theme.ready;
      current.requirements += theme.requirements;
      accumulator.set(theme.pillar, current);
      return accumulator;
    ...
    color:
      pillar === "Environmental" ? "var(--success)" : pillar === "Social" ? "var(--blue)" : "var(--warning)"
  }));
  const themeCoverage = disclosureThemes.map((theme) => {
    const progress = Math.round((theme.ready / theme.requirements) * 100);
    const tone =
      progress >= 75 ? "positive" : progress >= 50 ? "warning" : "critical";

    return {
      label: theme.theme,
      progress,
      valueLabel: `${theme.ready}/${theme.requirements}`,
      detail: `${theme.pillar} | ${theme.needsEvidence} need evidence`,
      tone
    } as const;
  });
  const themeFigureItems = disclosureThemes.map((theme) => ({
    ...
    color:
      theme.pillar === "Environmental" ? "var(--success)" : theme.pillar === "Social" ? "var(--blue)" : "var(--warning)"
  }));
  ...
  return (
    <AppShell
      ...
      subtitle="Manage framework requirements, narrative status, and non-environmental ESG disclosures."
    >
      <DashboardStatGrid
        items={[
          {
            label: "Binder readiness",
            value: `${Math.round((readyDisclosures / disclosures.length) * 100)}%`,
            detail: "Disclosure items marked ready",
            tone: readyDisclosures / disclosures.length > 0.65 ? "positive" : "warning"
          },
          {
            label: "In progress",
            value: `${inProgressDisclosures}`,
            detail: "Narratives and metrics in draft",
            tone: inProgressDisclosures > 0 ? "warning" : "neutral"
          },
          {
            label: "Non-env metrics",
            value: `${peopleMetrics.length + governanceRecords.length}`,
            detail: "People and governance indicators",
            tone: "neutral"
          }
        ]}
      />
      ...
      <section className="page-grid dashboard-row">
        <div className="page-section span-6">
          <DashboardMeterCard
            ...
            subtitle="Current state of the binder."
            items={[
              {
                label: "Ready",
                value: readyDisclosures,
                displayValue: `${readyDisclosures}`,
                detail: "Ready for reporting release",
                color: "var(--success)"
              },
              {
                label: "In progress",
                value: inProgressDisclosures,
                displayValue: `${inProgressDisclosures}`,
                detail: "Awaiting narrative completion",
                color: "var(--warning)"
              },
              {
                label: "Needs evidence",
                value: needsEvidenceDisclosures,
                displayValue: `${needsEvidenceDisclosures}`,
                detail: "Blocked by missing control support",
                color: "var(--critical)"
              }
            ]}
          />
        ...
        <div className="page-section span-6">
          <DashboardDonutCard
            title="Binder Figure"
            subtitle="Ready versus blocked disclosure lines."
            totalLabel={`${Math.round((readyDisclosures / Math.max(disclosures.length, 1)) * 100)}%`}
            items={[
              {
                label: "Ready",
                value: readyDisclosures,
      ...
      <section className="page-grid dashboard-row">
        <div className="page-section span-6">
          <DashboardMeterCard
            ...
            subtitle="Requirements grouped by ESG pillar."
            items={pillarRequirements}
          />
        </div>
        <div className="page-section span-6">
          <DashboardColumnChartCard
            title="Theme Figures"
            subtitle="Ready counts by disclosure theme."
            items={themeFigureItems}
          />
        </div>
      </section>
      ...
      <section className="page-grid dashboard-row">
        <div className="page-section span-12">
          <DashboardProgressCard
            ...
            subtitle="Topic-level readiness in the disclosure program."
            items={themeCoverage}
          />
        </div>
      </section>
              ...
              <tbody>
                {disclosures.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <Link className="entity-link" href={`/disclosures/${item.id}`}>
                        {item.title}
                      </Link>
                    </td>
